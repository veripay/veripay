        ...
        return (
            <div className="schedule-container">
                <div className="date-header">
                    <button className={"material-symbols-rounded"} onClick={this.handleDecrementDay}>arrow_back_ios
                    </button>
                    <FormatHeaderDate date={this.state.currentDay}/>
                    <button className={"material-symbols-rounded"} onClick={this.handleIncrementDay}>arrow_forward_ios
                    </button>
                </div>
                <div className="background-div">
                    ...
                    {this.props.database.getEventsOnDate(this.state.currentDay).map(ev => <Event {...ev} key={ev.id}/>)}
                </div>
            </div>
        )
...
function Event({name, start, end, payment, locationId, attended}) {
    ...
    return (
        <div className={"event-container " + (attended ? "attended" : (pastEvent ? "missed" : ""))}
            ...
            <p className="event-money"><b>{(attended ? "+ " : "") + `$${formatMoney(payment)}`}</b></p>
        </div>
    )
}
